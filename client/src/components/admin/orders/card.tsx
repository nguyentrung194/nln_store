...
import { DataOrder } from "../../../interfaces";
...
const headCellsOrder: HeadCell<DataOrder>[] = [
  {
    id: "id",
    numeric: false,
    disablePadding: true,
...
function EnhancedTableHead(props: EnhancedTableProps<DataOrder>) {
  const {
    onSelectAllClick,
    order,
    orderBy,
  ...
  const createSortHandler =
    (property: keyof DataOrder) => (event: React.MouseEvent<unknown>) => {
  ...
  return (
    <TableHead>
      <TableRow>
        ...
        {props.headCells.map((headCell: HeadCell<DataOrder>, index: any) => (
          <TableCell
            key={index}
            align={"center"}
...
export const OrderTable = ({ rows }: { rows: DataOrder[] }) => {
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        <EnhancedTableToolbarCustom
          ...
          selectOptionsLeft={[{ value: "delete", text: "Delete Orders" }]}
          selectOptions={[
            {
              key: "Add Order",
              text: "Add Order",
                          ...
                          optionsOrder={[
                            {
                              key: "Edit Order",
                              text: "Edit Order",
                              path: `/admin/orders/edit-order/${row.id}`,
                            },
                          ]}
                        />
                      </TableCell>
...
export const CardOrder = (props: any) => {
  return (
    <Card sx={{ minWidth: 0, height: "100%" }}>
      <CardContent>
        <OrderTable rows={props.values} />
      </CardContent>
    </Card>
  );
};
