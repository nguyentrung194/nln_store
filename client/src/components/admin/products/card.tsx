...
const headCellsProduct: HeadCell<DataProduct>[] = [
  {
    id: "id",
    numeric: false,
    disablePadding: true,
...
function EnhancedTableHead(props: EnhancedTableProps<DataProduct>) {
  const {
    onSelectAllClick,
    order,
    orderBy,
  ...
  const createSortHandler =
    (property: keyof DataProduct) => (event: React.MouseEvent<unknown>) => {
  ...
  return (
    <TableHead>
      <TableRow>
        ...
        {props.headCells.map((headCell: HeadCell<DataProduct>, index: any) => (
          <TableCell
            key={index}
            align={"center"}
...
export const ProductTable = ({ rows }: { rows: DataProduct[] }) => {
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] = React.useState<keyof DataProduct>("id");
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        <EnhancedTableToolbarCustom
          ...
          selectOptionsLeft={[{ value: "delete", text: "Delete Products" }]}
          selectOptions={[
            {
              key: "Add Product",
              text: "Add Product",
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        {JSON.parse(row.categories).map((el: string) => {
                          return el;
                        })}
                      </TableCell>
                          ...
                          optionsOrder={[
                            {
                              key: "Edit Product",
                              text: "Edit Product",
                              path: `/admin/products/edit-product/${row.id}`,
                            },
                          ]}
                        />
                      </TableCell>
...
export const CardProduct = (props: any) => {
  return (
    <Card sx={{ minWidth: 0, height: "100%" }}>
      <CardContent>
        <ProductTable rows={props.values} />
      </CardContent>
    </Card>
  );
};
